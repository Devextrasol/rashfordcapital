...
use Packages\Accounting\Models\User as User2;
...
class TradeService extends Service
{
    ...
    public function close(Trade $trade) {
        if ($trade->status == Trade::STATUS_OPEN) {
            ...
            //subhani code
            $myuser = User2::find($this->user->id);
            $paisa = (float) $trade->pnl;
            event(new AfterUserTradeClose($this->competition, $myuser, $paisa));
            //subhani code
