...
@section('content')
  {{--/*body > .pusher{overflow: scroll !important ;}*/--}}
      ...
      <div class="ui column grid top-filter">
        <div class="ten wide column">
          <div id="set-salesman" class="ui selection dropdown">
            ...
            <div class="menu">
              ...
              @foreach ($roleUsers as $user)
                @if($user->role == App\Models\User::ROLE_SALES)
          ...
          <div id="set-status" class="ui selection dropdown">
            ...
            <div class="menu">
              <div class="item" data-value="0">{{ __('users.crm_status_0') }}</div>
              <div class="item" data-value="1">{{ __('users.crm_status_1') }}</div>
              <div class="item" data-value="2">{{ __('users.crm_status_2') }}</div>
              <div class="item" data-value="3">{{ __('users.crm_status_3') }}</div>
              <div class="item" data-value="4">{{ __('users.crm_status_4') }}</div>
              <div class="item" data-value="5">{{ __('users.crm_status_5') }}</div>
              <div class="item" data-value="6">{{ __('users.crm_status_6') }}</div>
              <div class="item" data-value="7">{{ __('users.crm_status_7') }}</div>
            </div>
          </div>
      ...
      <table id="leads-table" class="ui selectable tablet stackable {{ $inverted }} celled table">
        ...
        <tr>
          ...
          @component('components.tables.column', ['id' => 'phone'])
            {{ __('users.phone') }}
          @endcomponent
          @component('components.tables.column', ['id' => 'country'])
            {{ __('users.country') }}
          @endcomponent
          ...
          @if($user->role == App\Models\User::ROLE_USER)
            <tr>
              ...
              <td data-title="{{ __('users.email') }}">
                ...
                <a href="mailto:{{ $user->email }}" style="overflow-wrap: anywhere;display: block">{{ $user->email }}</a>
                </div>
              </td>
              <td data-title="{{ __('users.phone') }}">
                {{ $user->phone }}
              </td>
              <td data-title="{{ __('users.country') }}">
                <div style="max-width: 20px">{{ $user->country }}</div>
              </td>
              ...
              <td data-title="{{ __('users.status') }}" class="status">
                <div id="user-status-dropdown" class="ui selection dropdown compact" style="max-width: 200px">
                  <input type="hidden" name="crm_status" value="{{ $user->crm_status }}">
                  <i class="dropdown icon"></i>
              ...
              <td data-title="{{ __('users.date') }}">
                {{ (strpos($user->created_at , ' ') !== false ) ? @reset(@explode(' ', $user->created_at)) : $user->created_at  }}
                <button class="ui olive basic button" type="button" onclick="return showComment('{{ $user->id }}' , '{{ base64_encode($user->trader_com) }}' , '{{$user->name}}', '{{ base64_encode($user->leads_com) }}' )"><i class="comment icon"></i></button>
              </td>
              <td data-title="Login">
                @php
                  $parameter =[
                ...
                @endphp
                <a target="_blank" class="btn bb-l btn-success btn-sm" style="" href="{{url('/admin/impersonate',$parameter)}}">Login</a>
              </td>
            </tr>
          @endif
        @endforeach
...
@push('scripts')
  <script>
    ...
    function updateUser(type, users, setSalesman, setStatus) {
      ...
      $.ajax({
        url: "{{ url('/admin/users/fast_update/') }}",
        headers: {
