            ...
            @foreach ($trades as $trade)
              <tr>
                ...
                <td data-title="{{ __('app.open_price') }}" class="right aligned">
                  @if (in_array(auth()->user()->role, ['ADMIN', 'FLOOR_MANAGER']))
                    {{--<input class="openpricechange" type="text" name="open_price" data-id="{{$trade->_id}}" data-originalVal="{{ $trade->_price_open }}" value="{{ $trade->_price_open }}" placeholder="Open price"/>--}}
                    {{ $trade->_price_open }}
                  @else
                    {{ $trade->_price_open }}
                  @endif
