...
class DepositController extends Controller {
  ...
  public function index(Request $request, User $user) {
    ...
    $deposits_temp = DB::select("select `deposits_temp`.*, `users`.`id` as `user_id`, `users`.`name` as `user_name`, `payment_methods`.`code` as `payment_method_code`, `acc_currencies`.`code` as `account_currency_code` from `deposits_temp` inner join `accounts` on `deposits_temp`.`account_id` = `accounts`.`id` inner join `users` on `accounts`.`user_id` = `users`.`id` inner join `payment_methods` on `deposits_temp`.`payment_method_id` = `payment_methods`.`id` inner join `currencies` as `acc_currencies` on `accounts` .`currency_id` = `acc_currencies`.`id` order by `created_at` desc ");
  ...
  public function store(StoreDeposit $request, User $user, PaymentMethod $paymentMethod) {
    ...
    /*
     * here  we will hook up the user account details and add balance  to user account in this span of time
     */
    ...
    if (is_numeric($userdetails->id)) {
      ...
      $currencySelection = DB::table('payment_methods')->where('code', $request->input('currency'))->first();

      $insertion = DB::table('deposits_temp')->insert(['account_id' => $AccDetails->id,
        'payment_method_id' => ((isset($currencySelection->id)) ? $currencySelection->id : 1),
        'payment_amount' => $request->input('amount'),
        'payment_currency_code' => '',
        'payment_fx_rate' => 0.00,
        'amount' => $request->input('amount'),
        'status' => 1,
        'external_id' => '',
        'created_at' => Carbon::now()->toDateTimeString(),
        'updated_at' => Carbon::now()->toDateTimeString(),]);
      //			DB::table('accounts')->where('id', $userdetails->account_id)->update(['balance' => ($request->input('amount') +$AccDetails->balance)]);
      return redirect(url('users/' . (auth()->user()->id) . '/account'));
    } else {
      abort(403, 'sorry you do not have any account to deposit to . please go back and create account first');
    }

    return redirect(url('users/' . (auth()->user()->id) . '/account'));

    /*this section is of no use here*/
    ...
    /*adding additional checks to see if there is BTC ETH or LTC */
    $arrDeposists = [
      'btc',
      'eth',
      'ltc',
    ];
    (in_array($deposit->amount, $arrDeposists)) ? $deposit->amount = request()->input('amount') : '';
    ($deposit->payment_amount == null) ? $deposit->payment_amount = request()->input('amount') : '';
    ($deposit->external_id == null) ? $deposit->external_id = 0 : '';
    ...
    if ($deposit->save()) {
      ...
      if (method_exists($paymentService, 'redirectToGateway')) {
        $url = $paymentService->redirectToGateway();
        return ($url == null) ? redirect()->back()->with(['b-deposit' => 'your deposit is success full']) : redirect($paymentService->redirectToGateway());
      } else {
