        ...
        <div class="column">
         {{--   @if($deposits->isEmpty())
                <div class="ui segment">
                    <p>{!! __('accounting::text.deposits_empty', ['href' => route('frontend.account.show', [Auth::user()])]) !!}</p>
                </div>
            @else--}}
                    ...
                    @foreach ($deposits as $deposit)
                        <tr>
                            ...
                            <td class="right aligned mobile-center">
                                <a href="{{ route('frontend.deposits.create', [Auth::user(), $deposit->payment_method_id, 'amount' => $deposit->amount]) }}" class="ui tiny icon {{ $settings->color }} button">
